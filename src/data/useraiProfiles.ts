...
import { AgeGroup, deriveAgeGroup, dbAgeGroupToUi, uiAgeGroupToDb } from '@/utils/age';
...
export class SupabaseProfilesRepo implements ProfilesRepo {
  ...
  private mapFromSupabase(row: any): UserAIProfile {
    return {
      ...
      ageGroup: dbAgeGroupToUi(row.age_group),
  ...
  private mapToSupabase(profile: UserAIProfile, ownerId: string) {
    return {
      ...
      age_group: uiAgeGroupToDb(profile.ageGroup),
